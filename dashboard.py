...
# Multiple options to load model
import pickle
import joblib
import cloudpickle
...
def load_model(filepath):
    if not os.path.exists(filepath):
        st.error(f"Model file tidak ditemukan: {filepath}")
        return None
...
def main():
    ...
    if st.sidebar.button('Prediksi Calon Nasabah'):
        if model:
            ...
            st.warning("Model gagal dimuat. Silakan periksa file model atau lingkungan dependency Anda.")
